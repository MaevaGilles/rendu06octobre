...
def question_3
    ...
    liste = data
    liste.select! {|i| i.length == 6}
    puts "il y a #{liste.length} qui ont 5 caractères."
end
...
def question_4
    ...
    liste = data
    puts "il y'a #{liste.grep(/[UPPERCASE]/).count} noms qui commencent par une majuscule !"
end
...
def question_6
    ...
    liste = data
    puts liste.sort_by!(&:length)
end

def question_7
    puts "Quelle est la position dans l'array de la personne @epenser ?"
    liste = data
    puts "@espenser est en #{liste.index("@epenser")}eme position"
end
